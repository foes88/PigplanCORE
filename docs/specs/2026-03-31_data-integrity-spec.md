# PigOS AI — 데이터 정합성 규격서

> 작성일: 2026-03-31
> 목적: 피그플랜 27년 운영에서 검증된 데이터 정합성 규칙을 PigOS AI 신규 스키마에 매핑
...
PigOS AI의 새로운 PostgreSQL 스키마에서도 **동일한 수준의 데이터 보호**가 이루어지도록 한다.
...
**PigOS AI 매핑** (piglet_events 테이블 필요):
```sql
-- 검증 함수
...
**PigOS AI 매핑** (breeding_cycles 테이블 사용 시):
```sql
-- 활성 사이클 1개 제한
...
**PigOS AI 매핑 — 이벤트 테이블별 자동 상태 전이 트리거**:
```sql
-- 교배 → GESTATING
...
**PigOS AI 매핑 — reproductive_events 확장**:
```sql
...
> 데이터 정합성 규칙을 PigOS AI 스키마에 매핑한 것입니다.
